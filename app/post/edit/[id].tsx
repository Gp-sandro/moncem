...
import { fetchPostById, updatePost, uploadCover } from '../../../lib/queries';
...
export default function EditPostScreen() {
  ...
  const [initialSnapshot, setInitialSnapshot] = useState<PostFormSnapshot | null>(null);
  ...
  useEffect(() => {
    ...
    fetchPostById(id)
      .then((post) => {
        ...
        setTitle(post.title);
        setBody(post.body ?? '');
        setMilestone(post.milestone ?? '');
        setTagsText(post.tags.join(', '));
        setExistingCoverUrl(post.coverUrl);
        setInitialSnapshot({
          type: post.type,
          title: post.title,
          body: post.body ?? '',
  ...
  const coverPreviewUri = newCoverUri ?? (!coverRemoved ? existingCoverUrl : null);
  const hasUnsavedChanges = initialSnapshot
    ? type !== initialSnapshot.type ||
      title !== initialSnapshot.title ||
      body !== initialSnapshot.body ||
      milestone !== initialSnapshot.milestone ||
      tagsText !== initialSnapshot.tagsText ||
      newCoverUri !== null ||
      coverRemoved ||
  ...
  async function handleSave() {
    ...
    if (noType || noTitle || tags.length > 5 || !userId || !id) return;
    ...
    try {
      ...
      if (newCoverUri) {
        coverUrl = await uploadCover(userId, newCoverUri, id);
    ...
    <SafeAreaView style={styles.safe} edges={['top']}>
      ...
      <View style={styles.header}>
        ...
        <Pressable
          ...
          style={[styles.headerBtn, (submitting || !title.trim()) && styles.saveDisabled]}
        >
          {submitting ? (
        ...
        <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
          ...
          <View style={styles.optionals}>
            ...
            <Pressable onPress={pickCover} style={styles.optionalRow}>
              {coverPreviewUri ? (
                <Image source={{ uri: coverPreviewUri }} style={styles.coverThumb} contentFit="cover" />
              ) : (
              ...
              <Text style={styles.optionalRowText}>
                {coverPreviewUri ? 'Cover image' : 'Add cover image'}
              </Text>
              {coverPreviewUri ? (
                <Pressable onPress={removeCover} hitSlop={8} style={styles.removeBtn}>
...
const styles = StyleSheet.create({
  ...
  typeBtnError: { borderColor: colors.errorRed },
  typeBtnText: {
    fontFamily: fonts.sansBold,
    fontSize: 11,
    letterSpacing: 0.8,
  ...
  typeBtnActiveText: { color: colors.white },
  inlineError: { fontFamily: fonts.sans, fontSize: 12, color: colors.errorRed },
  divider: {
    height: 1,
  ...
  titleInputError: { color: colors.errorRed },
  bodyInput: {
  ...
  coverThumb: { width: 40, height: 28, borderRadius: radius.tag },
  coverPlaceholder: {
    width: 40,
    height: 28,
  ...
  optionalIcon: { marginRight: spacing.md },
  optionalInput: {
    flex: 1,
    fontFamily: fonts.sans,
    fontSize: 14,
